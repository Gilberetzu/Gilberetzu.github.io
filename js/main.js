function todo(col, fil, numh, probH) {
    ...
    function renderingS(worldPixa) {
        for (var i = 0; i < worldPixa.length; i++) {
            if (worldPixa[i][0] == "pasto") {
                context.drawImage(pasto, worldPixa[i][1], worldPixa[i][2], S * (1 / sc), S * (1 / sc));
            } else if (worldPixa[i][0] == "hongo") {
                context.drawImage(hongo, worldPixa[i][1], worldPixa[i][2], S * (1 / sc), S * (1 / sc));
            }
        }
    }

    function deleteObj(worldRa) {
        for (var i = 0; i < worldRa.length; i++) {
            var ra = Math.sqrt((worldRa[i][1] - player1.posX) * (worldRa[i][1] - player1.posX) + (worldRa[i][2] - player1.posY) * (worldRa[i][2] - player1.posY));
            var angu = buscarAngulo(worldRa[i][1], worldRa[i][2], player1.posX, player1.posY);
            if (ra <= 25 + player1.radius /*&& (angu >= Math.PI/2 && angu <= Math.PI)*/ ) {
                hongoa[worldRa[i][3]] = ["hongo", -10000, -10000, worldRa[i][3]];
                puntos = puntos + 5;
